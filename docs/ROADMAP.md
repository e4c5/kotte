...
- Jobs: `lint` (ruff + black; mypy deferred B1.2), `unit` (pytest, no DB), `integration` (AGE service + `alembic upgrade head` + `pytest -m integration`, `USE_REAL_TEST_DB=true`).
...
- [~] **B1** — GitHub Actions: `backend-ci.yml` (PR #37 on `feat/b1-b2-ci-tests-and-lint`, bundled with B2; lint=ruff/black, unit, and integration jobs shipped; sub-ticket B1.2 below)
