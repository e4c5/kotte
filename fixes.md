...
## PR #16 — Review batch (2026-04-11) status

The following were **implemented** in the branch and the **20 open review threads** on PR #16 were resolved on GitHub with a [batch comment](https://github.com/e4c5/kotte/pull/16#issuecomment-4229580609):

- Optional `conn` for `get_backend_pid`; query execute + stream use one connection for PID + Cypher; `get_query_pid` uses one connection for pid + `pg_stat_activity`.
- Cypher `has_params = params is not None` (empty `{}` uses 3-arg `cypher()`).
- `execute_scalar` records DB query metrics.
- `analyze_pr.py` fails fast on GraphQL errors (no silent partial fetches).
- `SKILL.md` updated for `general` comments without `path`/`line`.

**Optional later (not blocking):** bound `fetchall`/`fetchone` with `asyncio.wait_for` for huge results; SonarCloud quality gate / hotspot triage in the Sonar UI; typing/docstring nits in `App.tsx`, `FilterTab`, and `test_security_headers.py`.
